...
void NaivePointcloudIntegrator::setupFromConfig(PointcloudIntegratorBase::Config *config) {
...
void NaivePointcloudIntegrator::processPointcloud(SubmapCollection *submaps,
  ...
  for (size_t i = 0; i < id_v.size(); ++i) {
    ...
    if (!tsdf_integrator_) {
      ...
      tsdf_integrator_.reset(new voxblox::FastTsdfIntegrator(config_.voxblox_integrator_config,
