 ...
 * In order to save memory, only the top N (parameter) class assignements are
 * serialized. Most of the times, voxel are only assigned 3-4 Classes, and there
 * is not much precision lost by not saving the others.
 *
...
// How many class assignements per voxel should be serialized if no parameter
// was provided #define PANOPTIC_MAPPING_DEFAULT_SERIALIZATION_COUNT int(4)
...
//  * Converts a Class Voxel into an int32 vector
//  */
// bool convertVoxelToInt32(const panoptic_mapping::ClassUncertaintyVoxel&
// voxel,
//                          std::vector<uint32_t>* data,
//                          int serialize_top_n_counts);
...
// /**
//  * This function is currently not implemented and only exists so voxblox can
//  compile
//  */
// template <>
