...
TEST(SubmapSave, serializeSubmapWithClassLayer) {
  ...
  std::unique_ptr<Submap> to_save_submap = getSubmap(true, true);
  std::fstream outfile;
  ...
  std::unique_ptr<Submap> to_load_submap = getSubmap(false, true);
  ...
  std::pair<const TsdfLayer, const ClassLayer> saved_layers =
      std::pair<const TsdfLayer, const ClassLayer>(
          to_save_submap->getTsdfLayer(), to_save_submap->getClassLayer());
  std::pair<const TsdfLayer, const ClassLayer> loaded_layers =
      std::pair<const TsdfLayer, const ClassLayer>(
  ...
  for (auto index : indexList) {
    // Check TSDF Layer
    check_block_equal(saved_layers.first.getBlockPtrByIndex(index).get(),
                      loaded_layers.first.getBlockPtrByIndex(index).get());

    // Check class layer
    check_block_equal(saved_layers.second.getBlockPtrByIndex(index).get(),
...
TEST(SubmapSave, serializeSubmapWithoutClassLayer) {
  ...
  std::unique_ptr<Submap> to_save_submap = getSubmap(true, false);
  ...
  std::ifstream infile;
  infile.open(tmpname, std::fstream::in | std::fstream::binary);
  std::unique_ptr<Submap> to_load_submap = getSubmap(false, false);
  uint64_t byte_offset = 0;
  to_load_submap = to_load_submap->loadFromStream(&infile, &byte_offset);
  infile.close();
  // Check equal
