...
TEST(ClassVoxel, serializeEmptyBlock) {
  ...
  block_to_save.serializeToIntegers(&data);
  block_to_load.deserializeFromIntegers(data);
...
TEST(ClassVoxel, moreThanTopNCounts) {
   ...
   * PANOPTIC_MAPPING_SERIALIZE_TOP_N_COUNTS) E.g. if
   * PANOPTIC_MAPPING_SERIALIZE_TOP_N_COUNTS = 3, only the top 3 counts are
  ...
  for (int i = 0; i < block_to_save.num_voxels(); i++) {
    if (i % 100 != 99) {
      ...
      EXPECT_LT(PANOPTIC_MAPPING_SERIALIZE_TOP_N_COUNTS, num_classes);
      for (int j = 0; j < PANOPTIC_MAPPING_SERIALIZE_TOP_N_COUNTS + 2; j++) {
        for (int k = 0; k <= j + 1; k++) {
...
TEST(BinaryClassVoxel, binaryClassifcation) {
  voxblox::Block<panoptic_mapping::ClassVoxel> block_to_save(
      voxels_per_side, voxel_size, origin);
  voxblox::Block<panoptic_mapping::ClassVoxel> block_to_load(
      voxels_per_side, voxel_size, origin);
...
TEST(BinaryClassUncertaintyVoxel, binaryClassifcation) {
  voxblox::Block<panoptic_mapping::ClassUncertaintyVoxel> block_to_save(
      voxels_per_side, voxel_size, origin);
  voxblox::Block<panoptic_mapping::ClassUncertaintyVoxel> block_to_load(
      voxels_per_side, voxel_size, origin);
