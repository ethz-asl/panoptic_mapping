...
def coco_labels():
    # Detectron 2 uses the coco labels, these are taken from
...
def generate_detectron_labels(out_file_name):
    with open(out_file_name, 'w') as csvfile:
        ...
        writer.writerow([
            "InstanceID", "ClassID", "PanopticID", "R", "G", "B", "Name",
            "SuperCategory"
        ])
        labels = coco_labels()
        for label in labels:
            writer.writerow([
                label["id"], label["id"], label["isthing"], label['color'][0],
                label['color'][1], label['color'][2], label['name'],
                label['supercategory']
            ])
...
if __name__ == '__main__':
    generate_detectron_labels(
        '/home/lukas/Documents/PanopticMapping/Data/labels/detectron_labels.csv'
    )
